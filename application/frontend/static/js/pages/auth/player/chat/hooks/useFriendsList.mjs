  ...
  async function updateFriendsList() {
    ...
    private_chats.forEach((chat) => {
      ...
      li.element.innerHTML = `
        ...
        <div class="d-flex flex-wrap gap-2 p-2 mt-2 container">
          <div class="col">
            <t-button id="profile-button" class="d-inline-block w-100 text-center" btn-class="w-100" theme="outlineInfo">Perfil</t-button>
          </div>
          <div class="col">
            <t-button id="chat-button" disabled="${chat.is_blocked}" class="d-inline-block w-100 text-center" btn-class="w-100" theme="outlineInfo">Conversar</t-button>
          </div>
          <div class="col">
            <t-button id="challenge-button" disabled="${friend.activity_status === "OFFLINE"}" class="d-inline-block w-100 text-center" btn-class="w-100"
          theme="outlineInfo">Desafiar</t-button>
          </div>
          <div class="col">
            <t-button id="toggle-block-button" class="d-inline-block w-100 text-center" btn-class="w-100" theme="danger">${chat.is_blocked ? "Desbloquear" : "Bloquear"}</t-button>
          </div>
        </div>
